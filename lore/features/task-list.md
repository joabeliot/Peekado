...
included) and shows them in a `List` — a circle/checkmark glyph, the title, an
optional time on the right. Open tasks sort first (by time), completed last.
Clicking a row marks it done: the glyph fills and the title strikes through in
place, then a PATCH goes to Notion. On failure it reverts and beeps. The footer
shows "X of Y done".
